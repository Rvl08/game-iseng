  ...
  return (
    <div
      ...
      style={{
        width: size,
        height: size,
        background: isPressed ? `radial-gradient(circle, ${color} 0%, ${color}cc 100%)` : `radial-gradient(circle, ${color}dd 0%, ${color}88 100%)`,
...
function GameCanvas({ world, players, zone, camera, isHost, particles, screenShake }: {
  world: number[][];
  players: Player[];
  ...
  useEffect(() => {
    ...
    for (let y = 0; y < WORLD_HEIGHT; y++) {
      for (let x = 0; x < WORLD_WIDTH; x++) {
        ...
        if (screenX < camera.x - TILE_SIZE || screenX > camera.x + width / camera.zoom ||
            screenY < camera.y - TILE_SIZE || screenY > camera.y + height / camera.zoom) continue;
    ...
    players.forEach(player => {
      ...
      const pattern = player.isAttacking ? SPRITE_PATTERNS.attack : player.isJumping ? SPRITE_PATTERNS.jump : SPRITE_PATTERNS.idle;
      ...
      ctx.fillRect(player.x - healthWidth / 2, player.y - 15, (player.health / player.maxHealth) * healthWidth, healthHeight);
    });
...
function HealthBar({ health, maxHealth }: { health: number; maxHealth: number }) {
  return (
    <div className="w-full h-3 rounded overflow-hidden" style={{ background: COLORS.healthBg, border: `1px solid ${COLORS.health}40` }}>
      <div className="h-full transition-all" style={{ width: `${(health / maxHealth) * 100}%`, background: `linear-gradient(90deg, ${COLORS.health} 0%, #ff6699 100%)` }} />
    </div>
  );
}
...
function InventorySlot({ item, isSelected, onClick }: { item: any; isSelected: boolean; onClick: () => void }) {
  return (
    <div onClick={onClick} className="w-12 h-12 flex items-center justify-center rounded cursor-pointer transition-all" style={{ background: isSelected ? COLORS.primary + '40' : 'rgba(255,255,255,0.1)', border: `2px solid ${isSelected ? COLORS.primary : COLORS.textDim + '40'}`, boxShadow: isSelected ? `0 0 10px ${COLORS.primary}` : 'none' }}>
...
function CountdownOverlay({ count }: { count: number }) {
  return (
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ background: 'rgba(0,0,0,0.5)' }}>
      <div className="text-9xl font-bold animate-ping" style={{ color: count === 0 ? COLORS.secondary : COLORS.accent, textShadow: `0 0 40px ${count === 0 ? COLORS.secondary : COLORS.accent}` }}>
        {count === 0 ? 'GO!' : count}
      </div>
    </div>
  );
  ...
  return (
    <div className="absolute top-4 left-4 flex flex-col gap-1">
      {kills.slice(-5).map(kill => (
        <div key={kill.id} className="px-3 py-1 rounded text-xs animate-fade-in" style={{ background: 'rgba(0,0,0,0.7)' }}>
...
function SettingsModal({ isOpen, onClose, settings, onSettingsChange }: { isOpen: boolean; onClose: () => void; settings: GameSettings; onSettingsChange: (s: GameSettings) => void }) {
  if (!isOpen) return null;

  const t = translations[settings.language];

  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 p-5" style={{ background: 'rgba(0,0,0,0.8)' }} onClick={onClose}>
      <div className="w-full max-w-md p-5 rounded-lg" style={{ background: COLORS.bg, border: `2px solid ${COLORS.primary}` }} onClick={e => e.stopPropagation()}>
        <h2 className="text-xl mb-5" style={{ color: COLORS.primary }}>{t.settings}</h2>

        <div className="flex flex-col gap-4">
          <div>
            <label className="text-xs block mb-2" style={{ color: COLORS.text }}>{t.masterVolume}: {settings.masterVolume}%</label>
            <input type="range" min="0" max="100" value={settings.masterVolume} onChange={e => { const v = parseInt(e.target.value); onSettingsChange({ ...settings, masterVolume: v }); audioManager.setMasterVolume(v / 100); }} className="w-full" />
          </div>

          <div>
            <label className="text-xs block mb-2" style={{ color: COLORS.text }}>{t.musicVolume}: {settings.musicVolume}%</label>
            <input type="range" min="0" max="100" value={settings.musicVolume} onChange={e => { const v = parseInt(e.target.value); onSettingsChange({ ...settings, musicVolume: v }); audioManager.setMusicVolume(v / 100); }} className="w-full" />
          </div>

          <div>
            <label className="text-xs block mb-2" style={{ color: COLORS.text }}>{t.sfxVolume}: {settings.sfxVolume}%</label>
            <input type="range" min="0" max="100" value={settings.sfxVolume} onChange={e => { const v = parseInt(e.target.value); onSettingsChange({ ...settings, sfxVolume: v }); audioManager.setSfxVolume(v / 100); }} className="w-full" />
          </div>

          <div>
            <label className="text-xs block mb-2" style={{ color: COLORS.text }}>{t.language}</label>
            <select value={settings.language} onChange={e => onSettingsChange({ ...settings, language: e.target.value as 'en' | 'id' })} className="w-full px-3 py-2 rounded" style={{ background: 'rgba(255,255,255,0.1)', border: `1px solid ${COLORS.textDim}`, color: COLORS.text }}>
        ...
        <button onClick={onClose} className="w-full mt-5 py-3 rounded cursor-pointer" style={{ background: COLORS.primary, color: '#fff' }}>{t.close}</button>
      </div>
    </div>
  );
}
...
export default function PixelClashGame() {
  ...
  if (currentPlayer.isDead) {
    return (
      <div className="w-full h-screen flex flex-col items-center justify-center gap-5 font-pixel" style={{ background: COLORS.bg }}>
        <div className="text-5xl">💀</div>
        <div className="text-base" style={{ color: COLORS.primary }}>{t.eliminated}</div>
        <div className="text-xs" style={{ color: COLORS.textDim }}>{t.youPlaced} #{alivePlayers.length + 1}</div>
        <div className="text-xs" style={{ color: COLORS.accent }}>{currentPlayer.kills} {t.kills}</div>
      </div>
    );
  }
